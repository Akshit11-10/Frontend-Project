...
const Results = () => {
  ...
  const getPerformanceMessage = () => {
    if (accuracy >= 90) return { message: 'Outstanding!', color: 'green', bg: 'bg-green-900/30', border: 'border-green-700' };
    if (accuracy >= 70) return { message: 'Great Job!', color: 'blue', bg: 'bg-blue-900/30', border: 'border-blue-700' };
    if (accuracy >= 50) return { message: 'Good Effort!', color: 'orange', bg: 'bg-orange-900/30', border: 'border-orange-700' };
    return { message: 'Keep Practicing!', color: 'red', bg: 'bg-red-900/30', border: 'border-red-700' };
  };
    ...
    return (
      <div className="min-h-screen bg-[#0f172a] py-8">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="bg-[#111827] rounded-2xl shadow-lg p-8 mb-8 border border-slate-700 animate-fadeInUp stagger-1">
          ...
          <div className="bg-[#111827] rounded-2xl shadow-lg p-6 md:p-8 mb-8 border border-slate-700 animate-fadeInUp stagger-2">
            ...
            <div className="space-y-4">
              {selectedReview.questions.map((q, index) => {
                ...
                const isExpanded = expandedReviewCards[q.id];

                return (
                  <div key={q.id} className={`border rounded-xl overflow-hidden transition-all ${isAnswered ? (isCorrect ? 'border-green-800 bg-green-900/20' : 'border-red-800 bg-red-900/20') : 'border-slate-600 bg-slate-800/30'}`}>
                    <div className="p-4 flex items-start gap-4">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${isAnswered ? (isCorrect ? 'bg-green-500 text-white' : 'bg-red-500 text-white') : 'bg-slate-600 text-white'}`}>
                      ...
                      <div className="flex-1">
                        ...
                        {isAnswered && <p className={`text-sm mt-2 ${isCorrect ? 'text-green-400' : 'text-red-400'}`}>Your answer: {q.options[userAnswer]}</p>}
                      </div>
                    </div>

                    <div className="px-4 pb-4 pl-12">
                      <button onClick={() => toggleReviewCard(q.id)} className="flex items-center gap-2 text-sm font-medium text-blue-400 hover:text-blue-300 transition-colors">
                      ...
                      {isExpanded && (
                        <div className="mt-4 p-4 bg-gradient-to-br from-blue-900/30 to-indigo-900/30 rounded-xl border border-blue-800/50 animate-fadeInUp">
                          <p className="text-slate-200 mb-2"><span className="font-medium">Correct Option:</span> <span className="text-green-400 font-medium">{q.options[q.correctAnswer]}</span></p>
...
<div className="bg-[#111827] rounded-2xl shadow-xl p-6 md:p-8 mb-8 border border-slate-700/50 animate-fadeInUp stagger-2 backdrop-blur-sm">
            <h2 className="text-2xl font-bold text-white mb-6 animate-fadeInUp stagger-2">Question Analysis</h2>
            <div className="space-y-4">
              {questions.map((q, index) => {
                ...
                return (
                  <div key={q.id} className={`border rounded-xl overflow-hidden transition-all duration-300 hover:border-blue-500/50 ${isAnswered ? (isCorrect ? 'border-green-800 bg-green-900/20' : 'border-red-800 bg-red-900/20') : 'border-slate-600 bg-slate-800/30'}`}>
                    <div className="p-4 flex items-start gap-4">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 transition-all duration-300 ${isAnswered ? (isCorrect ? 'bg-green-500 text-white' : 'bg-red-500 text-white') : 'bg-slate-600 text-white'}`}>
                        {isAnswered ? (isCorrect ? <CheckCircle className="h-5 w-5" /> : <XCircle className="h-5 w-5" />) : <span className="text-sm font-bold">{index + 1}</span>}
                      </div>
                      <div className="flex-1">
                        <p className="text-white font-medium">{q.question}</p>
                    ...
                    <div className="px-4 pb-4 pl-12">
                      <button onClick={() => toggleCard(q.id)} className="flex items-center gap-2 text-sm font-medium text-blue-400 hover:text-blue-300 transition-colors duration-300">
                        <Brain className="h-4 w-4 hover:scale-110 transition-transform duration-300" />
                        {isExpanded ? 'Hide' : 'Show'} AI Answer
                        {isExpanded ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
                      </button>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center animate-fadeInUp stagger-4">
          <Link to="/interview" className="flex items-center justify-center gap-2 px-8 py-4 bg-gradient-to-r from-blue-600 to-purple-600 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl hover:scale-[1.02] hover:-translate-y-1 transition-all duration-500 group">
            <RotateCcw className="h-5 w-5 group-hover:rotate-180 transition-transform duration-500" /> Try Another Interview
          </Link>
